    ...
    fs.createReadStream(filePath)
      ...
      .on('end', () => {
        try {
          ...
          t.deepEqual(results[0], { a: '1', b: '2', c: '3' }, 'First row should be parsed correctly')
          t.deepEqual(results[1], { a: '4', b: '5', c: 'ʤ' }, 'Second row should include Unicode character')
          resolve()
        } catch (error) {
          reject(error)
        }
    ...
    fs.createReadStream(filePath)
      ...
      .on('end', () => {
        try {
          t.is(results.length, 2, 'Should parse 2 rows')
          t.deepEqual(results[0], { a: '1', b: '2', c: '3' }, 'First row should be parsed correctly')
          t.deepEqual(results[1], { a: '4', b: '5', c: 'unicode-char' }, 'Second row should have mapped Unicode character')
          resolve()
        } catch (error) {
          reject(error)
        }
...
test('utf16 › UTF-16 BE with headers event', t => {
  return new Promise((resolve, reject) => {
    ...
    fs.createReadStream(filePath)
      ...
      .on('end', () => {
        try {
          t.deepEqual(headers, ['a', 'b', 'c'], 'Headers should be detected correctly')
          t.is(results.length, 2, 'Should parse 2 rows')
          t.deepEqual(results[0], { a: '1', b: '2', c: '3' }, 'First row should be parsed correctly')
          t.deepEqual(results[1], { a: '4', b: '5', c: 'ʤ' }, 'Second row should include Unicode character')
          resolve()
