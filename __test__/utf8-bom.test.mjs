      ...
      .on("end", () => {
        try {
          ...
          t.deepEqual(
            results[0],
            { name: "John", age: "30" },
            "First row should have clean headers without BOM",
          );
          t.deepEqual(
            results[1],
            { name: "Jane", age: "25" },
            "Second row should have clean headers without BOM",
          );
      ...
      .on("end", () => {
        try {
          t.deepEqual(
            headers,
            ["first", "second", "third"],
            "Headers should be clean without BOM",
          );
          t.is(results.length, 2, "Should parse 2 rows");
          t.deepEqual(
            results[0],
            { first: "a", second: "b", third: "c" },
            "First row should have clean headers",
          );
          t.deepEqual(
            results[1],
            { first: "1", second: "2", third: "3" },
            "Second row should have clean headers",
          );
